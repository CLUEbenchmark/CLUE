# 基于 PaddlePaddle 框架的 CLUE Benchmark

我们对 CLUE Benchmark 做了进一步扩展，基于 [PaddleNLP](https://github.com/PaddlePaddle/PaddleNLP) 实现了 [PaddlePaddle](https://www.paddlepaddle.org.cn/) 版本的 CLUE Benchmark。详情可点击 [PaddleNLP Benchmark 实现](https://github.com/PaddlePaddle/PaddleNLP/tree/develop/examples/benchmark/clue)查看。

如果计算资源充足，可以尝试 PaddleNLP 版本中提供的 Grid Search 功能（目前只支持 GPU 下直接使用），传入预训练模型的名称或者模型的路径即可一键启动，[批量启动 Grid Search](#批量启动GridSearch)后会自动寻找机器上空闲的 GPU 进行训练。最终可打印出每个数据集上最优的结果和对应的超参数。得到最好的模型后，可以参考下方[参加 CLUE 竞赛](#参加CLUE竞赛)步骤，用训好的模型进行预测并将结果提交至 [CLUE 榜单](https://www.cluebenchmarks.com/)，取得成绩和排行。
...
飞桨 PaddlePaddle 是源于产业实践的开源深度学习平台。推荐参考[飞桨官方文档](https://www.paddlepaddle.org.cn/)找到和机器环境符合的最新版本的安装命令并安装。

### 安装 PaddleNLP

同时需要安装最新版的 PaddleNLP 包，安装命令如下：
```
pip install --upgrade paddlenlp
```

运行训练代码时，PaddleNLP 会通过 API 调用自动下载内置模型或者数据集至 `~/.paddlenlp/models` 和 `~/.paddlenlp/datasets`，不需要手动下载数据/模型。如需更改路径，可通过修改环境变量 `PPNLP_HOME` 实现。
